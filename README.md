...
- 在[src/buried_core.h](src/buried_core.h)中具体定义Buried
- 在[src/buried.cc](src/buried.cc)中做一个桥接层，做C到C++的转换
...
- 在[src/buried_core.cc](src/buried_core.cc)中定义日志
- ```注意：由于我们使用了spdlog的自定义Formatter，所以需要定义以下宏，避免Custom Formatting异常：add_definitions(-DSPDLOG_ACTIVE_LEVEL=SPDLOG_LEVEL_TRACE)```
- 在[tests/test.cc](tests/test.cc)中写一个对应的单元测试，以测试日志功能
- ![image](https://github.com/user-attachments/assets/bf7ea2f0-5c0c-404d-b32f-0748ab263bde)
...
- 在[src/database/database.h](src/database/database.h)以及[src/database/database.cc](src/database/database.cc)中进行数据库设计
...
- **在该项目的具体实现**
- 使用Beast作为HTTP请求的库
- 在[tests/test_http.cc](tests/test_http.cc)中使用Beast发起HTTP请求
...
# AES加密解密算法
- 在[src/crypt/crypt.h](src/crypt/crypt.h)以及[src/crypt/crypt.cc](src/crypt/crypt.cc)中，生成一个用于AES加密的密钥
- 将随机盐值与用户提供的密码结合，再使用PBKDF2算法和HMAC-SHA256算法来生成密钥
- 将生成密钥转化为std::string类型返回
- 在[tests/test_crypt.cc](tests/test_crypt.cc)中，对加密解密方法进行测试

# strand多线程开发
- 避免任务并发执行时可能的数据竞争问题，确保任务顺序执行
- 相关代码：
- [src/context/context.h](src/context/context.h);
- [src/context/context.cc](src/context/context.cc);
- [examples/context_example.cc](examples/context_example.cc)
...
- 通过strand调度避免数据竞争
- 相关代码：
- [src/report/buried_report.h](src/report/buried_report.h)
- [src/report/buried_report.cc](src/report/buried_report.cc)

# 公共信息获取
- 获取用户id，系统版本号，设备id，设备名称，时间戳等信息
- 相关代码：
- [src/common/common_service.cc](src/common/common_service.cc)

# 整体功能组装
- 引入core模块，将各个子模块串联起来，使其协同工作，实现完整的埋点上报功能
- 相关代码：
- [src/buried_core.cc](src/buried_core.cc)
- [src/buried_core.h](src/buried_core.h)
